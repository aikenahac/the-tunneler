...
client.on('message', async (message) => {
  ...
  for (const channelId of channels) {
    ...
    const myWebhooks = webhooks.filter((webhook) => {
      ...
      return (
        webhook.owner.id === client.user.id && webhook.name === 'The Tunneler'
      );
    });
    ...
    console.log(
      `${message.author.username} [${serverName}] > ${messageToSend}`,
    );
      ...
      myWebhooks.first().send(messageToSend || 'No content provided', {
        username:
          member.nickname != null
            ? `${member.nickname}  [${serverName}]`
            : `${message.author.username} [${serverName}]`,
...
async function initWebhooks(channelId) {
  try {
    ...
    await channel.createWebhook('The Tunneler', {
      avatar:
        'https://cdn.discordapp.com/attachments/785272082136694824/816052706690334750/zerotwo.jpg',
    });
  } catch (err) {
